...
`src/tariffkit/data/`. There is no runtime network access, which means keeping
prices correct is entirely a matter of keeping that data current.

All of it is regenerable from a repository checkout through `tools.regen`.
Generators are maintainer programs rather than runtime behavior, so they are
strictly checked here but excluded from wheel and sdist.

```bash
python -m tools.regen                      # tariffs, export adders, CCA rate cards
python -m tools.regen --check              # exit 1 if anything upstream has moved
python -m tools.regen tariff --provider eelec
python -m tools.regen cca --provider mce --pdf ~/Downloads/mce-rates.pdf
python -m tools.regen.export --download   # the 843 MB export archive
```
...
Publishers are declared in `tools/regen/providers.py`. PG&E is not the
...
1. `python -m tools.regen cca` reports `SOURCE CHANGED` and names the cached PDF.
...
`src/tariffkit/data/export/pge/*.json.gz`, `holidays.toml`, and
`manifest.json` are all generated by `tools.regen.export` from PG&E's
...
`src/tariffkit/data/tariff/pge/*/*.toml` are generated by
`python -m tools.regen tariff` from PG&E's published tariff sheets. **Do not hand-edit
the rate tables**; regenerate them.

```bash
python -m tools.regen tariff
python -m tools.regen tariff --provider eelec --pdf /path/to/sheet.pdf
python -m tools.regen tariff --check   # CI-style
```
...
2. **The library must be able to price what was generated.** The generator
   writes key names and `tariffkit.tariff.retail` reads them back, two
...
```bash
python -m tools.regen tariff --advice-letter 7797-E
```
