...
This module is what goes around it for an account: checking the run about to
be folded is contiguous, deciding which arrangement the account is under, and
refusing with a reason where the inputs cannot support an answer.

It lived in the Home Assistant integration until the CLI needed the same
answer. `tariffkit bill` priced a cycle with an empty bank while the
integration showed a carried one, from the same account; two front ends
disagreeing about money is the thing this module exists to prevent.
"""
...
from .ledger import CreditBalances
from .models import Bill
from .trueup import run_lifetime
