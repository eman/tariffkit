...
rather than the pre-aggregated buckets :mod:`nem_rates.sources.homeassistant`
    ...
    def load(
        ...
        **overrides: str | None,
        ...
        for key, name in (
            ...
            ("import_entity", "NEM_RATES_INFLUX_IMPORT_ENTITY"),
            ("export_entity", "NEM_RATES_INFLUX_EXPORT_ENTITY"),
        ):
...
def _query(settings: InfluxSettings, sql: str) -> list[dict[str, Any]]:
    try:
        ...
        raise RuntimeError(
            "the InfluxDB source requires the 'influx' extra: pip install 'nem-rates[influx]'"
        ) from exc
