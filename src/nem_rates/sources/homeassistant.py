...
``websockets`` and lives outside :mod:`nem_rates.billing`, which is deliberately
    ...
    def load(
        ...
        **overrides: str | None,
        ...
        if token := env.get("HA_TOKEN"):
            values["token"] = token
        for key, name in (
            ("import_entity", "NEM_RATES_HA_IMPORT_ENTITY"),
            ("export_entity", "NEM_RATES_HA_EXPORT_ENTITY"),
        ):
...
async def _fetch(
    ...
    try:
        ...
        raise RuntimeError(
            "the Home Assistant source requires the 'ha' extra: pip install 'nem-rates[ha]'"
        ) from exc
