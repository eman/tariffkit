...
Vendored data is regenerated by `tools/regen_data.py`, which downloads PG&E's
archive, converts each row from UTC to Pacific time before bucketing, and
asserts that every one of the 576 cells per year resolves to exactly one value.

```bash
python tools/regen_data.py --download            # rewrite the vendored data
python tools/regen_data.py --download --check    # exit 1 if upstream moved
```
