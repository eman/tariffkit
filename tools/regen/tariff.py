...
advice letter, so a stale snapshot surfaced only when a bill disagreed.
    ...
    python -m tools.regen tariff --check
    python -m tools.regen tariff --provider etouc --pdf /path/to/sheet.pdf
...
def render(
    ...
    lines = [
        ...
        f"# Source: {url}",
        "#",
        "# GENERATED by `python -m tools.regen tariff` -- do not hand-edit the rate tables.",
        "# Regenerate with:",
        f"#     python -m tools.regen tariff --provider {slug}",
        "#",
...
def price_through_the_loader(slug: str, body: str, data: Extracted, provider: Utility) -> list[str]:
    ...
    ``render`` writes key names as string literals; ``tariffkit.tariff.retail``
    ...
    from tariffkit.tariff import retail
...
def _pcia_from_earlier_filing(
    ...
    root = (cache or Path.home() / ".cache" / "tariffkit" / "regen") / "al"
...
def _franchise_fees(
    ...
    root = cache or Path.home() / ".cache" / "tariffkit" / "regen"
