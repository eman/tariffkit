...
pypdf is a maintainer dependency. Nothing on the pricing path reaches this
...
class Page:
    ...
    :func:`tools.regen.tariff.pick_effective` for why that distinction
...
def read_pages(path: Path) -> list[Page]:
    ...
    try:
        ...
        raise ExtractionError(
            "regenerating rate data needs the maintainer dependencies: run `uv sync --group regen`"
        ) from exc
...
def _require_text_layer(path: Path, pages: list[Page], raw_pages: Sequence[Any]) -> None:
    ...
    **A document with no text layer is not a dead end.** It cannot be *parsed*,
    ...
    :func:`tools.regen.cca._watch_by_checksum`.
    """
