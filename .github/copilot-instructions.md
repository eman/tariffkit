...
- **Python 3.14.2 is the floor**, and CI runs exactly that one version rather than a matrix — a matrix that spanned versions nobody develops on could only catch regressions where they did not matter. The Home Assistant floor in `hacs.json` (currently 2026.8.0) is set separately, by the newest Home Assistant API the integration calls.
